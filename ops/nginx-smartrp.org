...
server {
    ...
    location = /api/progress/healthz { alias /srv/smartrp/frontend/dist/healthz.json; default_type application/json; add_header Cache-Control "no-store" always; }
    ...
    # JS – viena Cache-Control, be Expires
    location ~* \.js$ {
        try_files $uri =404;
        expires off;
